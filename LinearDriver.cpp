...
*/
   ...
   const int size = 40000;
    ...
    int search_index = 100;
        ...
        HashTable HT(40009); // creates a HashTable os size 40,009
       ...
       // cout << "Time it takes to use Linear Probe" << endl;
            ...
            for(int i=0; i < 400; ++i)
            {
                start = high_resolution_clock::now();
                    ...
                    for(int j=0; j<=100; j++)
                    {
                        ...
                        HT.LinearSearch(testData[index]);
                    }
                    search_index += 100;
                    end = high_resolution_clock::now();
                    execTime = duration_cast<microseconds>(end-start);
        ...
        ofstream myFile("LPC.csv");
    for(int i=0; i < 400.0; ++i)
    {
        myFile << i << ", " << HT.getNumOfCollision() << endl;
    }
